...
### DeepSeek API 401/402/403

检查 Key、账户余额和服务状态；不要把完整错误中的敏感内容复制到公开 Issue。
...
默认只把前 12 个候选和每个最多 2500 字符的 README 摘要发给模型。先记录 7 天的 `usage`，再比较 `deepseek-v4-flash` 的思考开关、`deepseek-v4-pro` 和可选 OpenAI Provider；只有质量评测不下降时才切换。不要仅凭单次主观感觉选择模型。
